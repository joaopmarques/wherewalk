...
/** Converts a Target to the distance that WhereWalk plans for, in meters. */
export function targetToMeters(target: Target, settings: Pick<WalkerSettings, 'paceKmh' | 'strideM'>): number {
  switch (target.kind) {
    case 'distance':
      return target.value
    case 'time':
