...
export interface Router {
  ...
  path(from: LngLat, to: LngLat): Promise<{ coordinates: LngLat[]; lengthM: number }>
}
...
const stopsPlanning = (error: unknown) =>
  typeof error === 'object' && error !== null && (error as { stopsPlanning?: unknown }).stopsPlanning === true
...
export async function planRoutes(
  ...
  try {
    ...
    if (e.fits.length > 0) return { kind: 'candidates', candidates: sortByCloseness(e.fits, targetM) }
    throw e.error
  }
}
...
const sortByCloseness = (routes: Route[], targetM: number) =>
  [...routes]
    .sort((a, b) => Math.abs(a.lengthM - targetM) - Math.abs(b.lengthM - targetM))
    .slice(0, MAX_CANDIDATES)

async function plan(origin: LngLat, targetM: number, router: Router, seed: number): Promise<PlanResult> {
  ...
  const run = async (attempts: Attempt[]): Promise<Route[]> => {
    ...
    for (let round = 0; round <= CORRECTION_ROUNDS; round++) {
      const pending = round === 0 ? attempts : attempts.filter((a) => a.last && !fitsTarget(a.last.lengthM, targetM))
      if (pending.length === 0) break
      if (round > 0) for (const a of pending) a.size *= targetM / a.last!.lengthM

      const results = await Promise.allSettled(pending.map((a) => a.make(a.size)))
      results.forEach((r, i) => {
      ...
      const fatal = results.find((r) => r.status === 'rejected' && stopsPlanning(r.reason))
      if (fatal) throw new PlanningStopped((fatal as PromiseRejectedResult).reason, distinct(fits))
      if (distinct(fits).length >= ENOUGH_CANDIDATES) break
    }
    return distinct(fits)
  }

  const toResult = (fits: Route[]): PlanResult => ({ kind: 'candidates', candidates: sortByCloseness(fits, targetM) })
  ...
  const outAndBacks = await run(
    Array.from({ length: MAX_CANDIDATES }, (_, i) => {
      ...
      return {
        ...
        make: async (reach: number) => toOutAndBack(await router.path(origin, destination(origin, reach, bearing))),
      }
    }),
  )
...
function toOutAndBack(path: { coordinates: LngLat[]; lengthM: number }): Route {
  ...
  return { shape: 'out-and-back', coordinates: [...path.coordinates, ...back], lengthM: path.lengthM * 2 }
}
