...
export function measureRoute(coordinates: LngLat[]): MeasuredRoute {
  ...
  for (let i = 1; i < coordinates.length; i++) {
    cumulativeM.push(cumulativeM[i - 1] + distanceM(coordinates[i - 1], coordinates[i]))
  }
  return { coordinates, cumulativeM, lengthM: cumulativeM[cumulativeM.length - 1] }
}
 ...
 */
export function matchProgress(route: MeasuredRoute, position: LngLat, previousM: number): number {
  ...
  for (let i = 0; i < coordinates.length - 1; i++) {
    ...
    const { t, offsetM } = projectOnSegment(position, coordinates[i], coordinates[i + 1])
    const alongM = Math.max(previousM, cumulativeM[i] + t * (cumulativeM[i + 1] - cumulativeM[i]))
  ...
  for (const m of matches) {
    if (m.offsetM <= bestOffset + TIE_OFFSET_M) earliest = Math.min(earliest, m.alongM)
  }
...
export function isFinished(route: MeasuredRoute, position: LngLat, progressM: number, origin: LngLat): boolean {
  if (progressM < route.lengthM / 2) return false
  return distanceM(position, origin) <= FINISH_RADIUS_M || progressM >= route.lengthM - FINISH_RADIUS_M
}
