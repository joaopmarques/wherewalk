...
import { isFinished, matchProgress, measureRoute, sliceRoute } from './progress'
...
describe('Target conversion', () => {
  it('converts time, distance, and steps to meters', () => {
    expect(targetToMeters({ kind: 'time', value: 30 }, settings)).toBeCloseTo(2500)
    expect(targetToMeters({ kind: 'distance', value: 3000 }, settings)).toBe(3000)
    expect(targetToMeters({ kind: 'steps', value: 5000 }, settings)).toBeCloseTo(3750)
  })
...
describe('Progress', () => {
  ...
  it('finishes only after halfway, back near the Origin', () => {
    ...
    expect(isFinished(route, destination(origin, 250, 0), 600, origin)).toBe(false)
  })
...
describe('Route planner', () => {
  ...
  function fakeRouter(loopFactors: number[], pathFactor = 1.3) {
    ...
    const router: Router = {
      loop: async (o, lengthM, seed) => {
        ...
        return { shape: 'loop', coordinates: [o, mid, o], lengthM: lengthM * loopFactors[seed % loopFactors.length] }
      },
      path: async (from, to) => {
        calls.path++
        return { coordinates: [from, to], lengthM: distanceM(from, to) * pathFactor }
      },
    }
    return { router, calls }
  }
  ...
  it('returns fitting Loops sorted by closeness to the Target', async () => {
    ...
    expect(result.candidates.map((r) => Math.round(r.lengthM))).toEqual([3060, 2910, 3150])
  ...
  it('corrects Loops that come back much too long', async () => {
    ...
    expect(result.candidates.every((r) => r.shape === 'loop' && fitsTarget(r.lengthM, 2500))).toBe(true)
    expect(calls.loop).toBe(10)
    expect(calls.path).toBe(0)
  })

  it('removes duplicate Loops from different seeds', async () => {
    const same: Route = { shape: 'loop', coordinates: [origin, [1, 1], origin], lengthM: 3000 }
    const router: Router = { loop: async () => same, path: () => Promise.reject(new Error('unused')) }
    const result = await planRoutes(origin, 3000, router)
  ...
  it('falls back to Out-and-backs when no Loop fits', async () => {
    ...
    expect(result.kind).toBe('candidates')
    if (result.kind !== 'candidates') return
    expect(result.candidates.every((r) => r.shape === 'out-and-back')).toBe(true)
    expect(result.candidates.every((r) => fitsTarget(r.lengthM, 3000))).toBe(true)
  })
  ...
  it('returns the Closest Route when nothing fits', async () => {
    ...
    let call = 0
    const router: Router = {
      loop: async (o, _lengthM, seed) => ({ shape: 'loop', coordinates: [o, [seed, seed], o], lengthM: lengths[call++ % 3] }),
  ...
  it('keeps Candidates found before an error that stops planning', async () => {
    ...
    const router: Router = {
      loop: async (o, lengthM, seed) => {
        ...
        return { shape: 'loop', coordinates: [o, [seed, seed], o], lengthM: seed === 0 ? lengthM : lengthM * 3 }
      },
