 ...
 */
export function useAnimatedHeight<Box extends HTMLElement, Content extends HTMLElement>() {
  ...
  useLayoutEffect(() => {
    ...
    const fit = () => {
      ...
      const height = content.offsetHeight + parseFloat(style.paddingTop) + parseFloat(style.paddingBottom)
    ...
    const onTransitionEnd = (e: TransitionEvent) => {
      if (e.target === box && e.propertyName === 'height') box.classList.remove('is-resizing')
    }
