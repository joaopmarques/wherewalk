...
export function RouteShield({ label = '2', size = 36 }: { label?: string; size?: number }) {
  const clipId = useId()
  return (
    <svg className="shield" viewBox="0 0 40 44" width={size} height={size * 1.1} aria-hidden="true">
      <defs>
        <clipPath id={clipId}>
          <path d={INNER} />
        </clipPath>
      ...
      <text x="20" y="35.5" textAnchor="middle" fontSize="19" fontWeight="800" fill="#fff" fontFamily="Overpass, sans-serif">
        {label}
      </text>
    </svg>
  )
