...
export const metersPerUnit = (units: Units) => (units === 'imperial' ? M_PER_MI : M_PER_KM)
...
export function formatMinutes(minutes: number): string {
  ...
  const m = rounded % 60
  return m === 0 ? `${rounded / 60} h` : `${Math.floor(rounded / 60)} h ${m} min`
}

export const formatSteps = (steps: number) => Math.round(steps).toLocaleString()
