...
WhereWalk plans a walk that starts and ends at one point, usually where the Walker is, sized to how much the Walker wants to walk right now.

## Language

**Walker**:
The person who uses WhereWalk to plan and follow a Route.
...
**Loop**:
A Route that goes out on one set of streets and comes back on different streets. WhereWalk tries for a Loop first.
_Avoid_: Circuit, round trip

**Out-and-back**:
A Route that goes to a far point and returns on the same streets. WhereWalk uses it only when it cannot find a Loop near the Target.
_Avoid_: Return trip, there-and-back

**Target**:
How much the walker wants to walk, given as a time, a distance, or a step count. WhereWalk converts every Target to a distance before it plans a Route.
_Avoid_: Goal, budget, amount

**Pace**:
The walker's walking speed. WhereWalk uses it to convert a time Target to a distance.
_Avoid_: Speed, tempo

**Stride**:
The length of one of the walker's steps. WhereWalk uses it to convert a step-count Target to a distance.
...
**Candidate**:
A Route inside the Tolerance that WhereWalk offers to the Walker. Each Candidate has its own color.
...
**Closest Route**:
A Route outside the Tolerance. WhereWalk shows it, with its real length, only when it finds no Candidate.
